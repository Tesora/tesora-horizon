...
from openstack_dashboard.dashboards.project.database_clusters \
    import cluster_manager
from openstack_dashboard.dashboards.project.database_clusters import tables
...
class ClustersTests(test.TestCase):
    @test.create_stubs({api.trove: ('cluster_list',
                                    'flavor_list')})
    def test_index(self):
        clusters = common.Paginated(self.trove_clusters.list())
        api.trove.cluster_list(IsA(http.HttpRequest), marker=None)\
            .AndReturn(clusters)
    ...
    def test_create_simple_vertica_cluster(self):
    ...
    @test.create_stubs({api.trove: ('cluster_get',
                                    'instance_get',
                                    'flavor_get',)})
    ...
    def test_grow_cluster(self):
        ...
        api.trove.cluster_grow(IsA(http.HttpRequest),
                               cluster.id,
                               instances)
        self.mox.ReplayAll()
    ...
    def test_shrink_cluster(self):
        ...
        api.trove.cluster_shrink(IsA(http.HttpRequest),
                                 cluster.id,
                                 cluster_instances)
        self.mox.ReplayAll()
